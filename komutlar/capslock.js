...
exports.run = async (client, message, args) => {
  ...
  if (message.author.id !== ayarlar.sahip) return message.channel.send(`Capslock adlı komutu kullanabilmek için bot sahibine ulaş.`)
  ...
  if (capslock) {
    ...
    message.channel.send(`Capslock engelleme sistemi, kapatıldı! :x:`)
  }
 
  if (!capslock) {
    db.set(`capslock_${message.guild.id}`, 'acik')
    message.channel.send(`Capslock engelleme sistemi, aktif! :white_check_mark:`)
  }
};
exports.conf = {
  enabled: true,
